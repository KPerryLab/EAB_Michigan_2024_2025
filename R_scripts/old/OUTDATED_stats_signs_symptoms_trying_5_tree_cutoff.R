...
# ****Testing whether the results change if I switch the analysis so that
# trees are included if they belong to a plot with more than 5 ash trees,
# instead of the previous cutoff at 10 trees.******
...
# consider trees within Plots where more than *****5***** trees were found.
...
sum(ash_by_plot$number_trees > 5, na.rm=TRUE) # There are 12 plots where total 
# number of trees is greater than 5 (there were only 9 plots where total
# number of trees is greater than 10)
plots_for_sign_symptom_models <- ash_by_plot[which(ash_by_plot$number_trees > 5), 
                                             "center_tree_number"]
...
# This sub-setting by the 5 tree cutoff reduces the sample size to 294 trees. 
# Previously, the 10 tree cutoff reduced the sample size to 274 trees.
...
# ash tree death (0 or 1)
# ash tree decline (0 or 1)
